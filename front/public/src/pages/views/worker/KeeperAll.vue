    ...
    <div v-for="post in Views.workerKeeper" :key="post.id">
        <div class="anketa">
            ...
            <div class="anketa_content">
                ...
                <div class="anketa_content_about"> {{ post[0].keeper_exp }} </div>
...
export default {
    ...
    methods: {
        showItem(item) {
            this.Views.workerKeeperitemUser = item[0].User;
            localStorage.workerKeeperitemUser = JSON.stringify(item[0].User); 

            this.Views.workerKeeperitem = item[0];
            localStorage.workerKeeperitem = JSON.stringify(item[0]);
