<template>    
    ...
    <div v-if="totalPages > 1" class="pagination">
        <button class="pagination_btn" @click="prevPage" :disabled="Views.currentCNPage === 1">Назад</button>
        <span>{{ Views.currentCNPage }}</span>
        <button class="pagination_btn" @click="nextPage" :disabled="Views.currentCNPage === totalPages">Вперед</button>
    </div>
</template>
...
export default {
    ...
    methods: {
        watch() {
            this.Views.WATCH_CLIENTNURSE(localStorage.userID);
        },
        search() {
            this.Views.SEARCH_CLIENTNURSE(this.searchData);
        },
        clear() {
        ...
        nextPage() {
            if (this.Views.currentCNPage < this.totalPages) { this.Views.currentCNPage++; }
        },
        prevPage() {
            if (this.Views.currentCNPage > 1) { this.Views.currentCNPage--; }
        },
    },
    computed: {
        displayedPosts() {
